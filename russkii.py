'''import pygame
#https://www.youtube.com/watch?v=091ixsTn4HM
...
fps = 25
window_w, window_h = 600, 500 # Canvas massen
block, cup_h, cup_w = 20, 20, 10 # grosse block(20*20 Picksel),cup_h, cup_w in block


side_freq, down_freq = 0.15, 0.1 # передвижение в сторону и вниз

side_margin = int((window_w - cup_w * block) / 2)
# side_margin  Abstand bei von Canvas und Glas
...
fig_w, fig_h = 5, 5#figure massen
empty = 'o'#auf "o" wert gestellt
'''Beispiel für dictonary (set ist auch mit {} aber andere eigenschaften)
thisdict = {
  "brand": "Ford",
  "electric": False,
  "year": 1964,
...
print(thisdict["colors"][2])>>> ausgabe :blue'''
...
figures = {'S': [['ooooo',
           ...
           'T': [['ooooo',
                 ...
                 ['ooooo',
                  ...
                  'oxxoo',
                  'ooxoo',
                  'ooooo']]}#mittestehende x wird als achse zur rotirung
'''поворачивать на 90 градусов, все возможные варианты 
поворотов описаны в словаре figures с помощью вложенных 
списков, элементы которых состоят из строк: символом x 
отмечены занятые ячейки, o – пустые. Количество вращений 
зависит от формы буквы:
 у O, к примеру, будет всего один вариант:
 Поскольку каждая фигура состоит из 4 блоков,
  размер шаблона должен быть 5 х 5: fig_w, fig_h = 5, 5.(oben 
  geschriben)'''
def pauseScreen():
        pause = pg.Surface((600, 500), pg.SRCALPHA)#SRCALPHA bischen transparent fon macht
        pause.fill((0, 0, 255, 127)) # das ist color + alfa прозрачностьnotice the alpha value in the color
        display_surf.blit(pause, (0, 0))# обьект и начала заливки старого экрана новым
...
def getNewFig():
    ...
    shape = random.choice(list(figures.keys()))#СЛОВАРЬ : ЧЕРЕЗ ЭТО БУКВА НА CHOICE() ПОПАДАЕТ ДА И ВЕЗДЕ ГДЕ : ТАМ СЛОВАРЬ
    newFigure = {'shape': shape,                                      #СЛОВАРЬ :
                'rotation': random.randint(0, len(figures[shape]) - 1), # 1 назад помоему в figures[shape]) - 1 посмотреть
...
if __name__ == '__main__': #https://pythonru.com/uroki/funkcija-main-v-python-dlja-nachinajushhih
    main()
