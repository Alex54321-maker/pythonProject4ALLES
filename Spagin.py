...
#https://www.youtube.com/watch?v=fI2jhAWtodY
W, H = 10, 15#wieviel plitka passen in W(breite) und H(ohe)
TILE = 40#masse der plitka
...
def on_closing():
    global app_running
    if messagebox.askokcancel("Raus","Möchten Sie raus?"):
        app_running = False
       # tk.destroy()
...
grid = [game_sc.create_rectangle(x*TILE,y*TILE,x*TILE+TILE,y*TILE+TILE) for x in range(W)for y in range(H)]#Rect(x координаты,y координаты,ширина, высота)
...
sc.create_text(505, 30,text="TETRIS", font=( 50),fill="black", anchor=NW)
sc.create_text(535, 360,text="score:", font=( 35),fill="black", anchor=NW)
_score = sc.create_text(550, 460,text=str(score), font=( 35),fill="black", anchor=NW)
sc.create_text(525, 650,text="record:", font=( 35),fill="black", anchor=NW)
_record=sc.create_text(550, 660,text=record, font=( 35),fill="black", anchor=NW)
...
'''for item in grid:
    canvas.itemconfigure(item, fill= "")'''# Tile ohne farbe
'''
#draw figure
for i in range(4):
...
def check_borders():
    if figure[i][0] < 0 or figure[i][0] > W - 1:
        ...
        return False
    print(field[figure[i][1]],"i and 1")#[figure[i][0]],"ddd",field[figure[i][1]-1],"ddd"[figure[i][0]-1])
    print(field[figure[i][0]],"i and 0")
    return True
...
while app_running:
    ...
    if app_running:
        ...
        dx, rotate = 0,False
        tk.update_idletasks()# idle -leerlauf übersetzung
        tk.update()
